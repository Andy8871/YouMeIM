...
{
  struct SessionHandle *data = conn->data;
  struct HTTP *http = (HTTP*)data->req.protop;
                           ...
                           void *userp)
{
  ...
  struct HTTP *http = (HTTP*)conn->data->req.protop;
  ...
  conn->data->req.forbidchunk = (http->sending == HTTP::HTTPSEND_REQUEST) ? TRUE : FALSE;
  ...
  if(http->postsize <= (curl_off_t)fullsize) {
    ...
    if(http->backup.postsize) {
	  ...
	  http->sending = (enum HTTP::eSendMENU)((int)http->sending + 1); /* move one step up */
  ...
  if(conn->bits.httpproxy && !conn->bits.tunnel_proxy)  {
    ...
    if(checkprefix("ftp://", ppath)) {
      if(data->set.proxy_transfer_mode) {
       ...
       const char *type = strstr(ppath, ";type=");
