...
Curl_printable_address(const Curl_addrinfo *ai, char *buf, size_t bufsize)
{
  ...
  switch (ai->ai_family) {
    case AF_INET:
      sa4 = (const sockaddr_in *)ai->ai_addr;
                ...
                int port)
{
  ...
  dns =(Curl_dns_entry*) Curl_hash_pick(data->dns.hostcache, entry_id, entry_len+1);
                ...
                int port)
{
  ...
  dns2 = (Curl_dns_entry*)Curl_hash_add(data->dns.hostcache, entry_id, entry_len + 1,
                       (void *)dns);
  if(!dns2) {
    free(dns);
    free(entry_id);
