...
CURLcode Curl_dupset(struct SessionHandle *dst, struct SessionHandle *src)
{
  ...
  for(int i= 0; i< STRING_LASTZEROTERMINATED; i++) {
...
CURLcode Curl_open(struct SessionHandle **curl)
{
  ...
  data =(SessionHandle*) calloc(1, sizeof(struct SessionHandle));
  if(!data) {
  ...
  *protocol_done = FALSE;
  ...
  if(!conn->bits.protoconnstart) {
    ...
    if(conn->bits.tunnel_proxy && conn->bits.httpproxy &&
		(conn->tunnel_state[FIRSTSOCKET] != connectdata::TUNNEL_COMPLETE))
...
{
	struct connectdata *conn = (connectdata*)calloc(1, sizeof(struct connectdata));
  if(!conn)
    return NULL;
                                    ...
                                    char **optionsp)
{
  ...
  *prot_missing = FALSE;
  ...
  if(rebuild_url) {
	...
	reurl = (char*)malloc(urllen + 2); /* 2 for zerobyte + slash */
    if(!reurl)
      return CURLE_OUT_OF_MEMORY;
    ...
    if(!conn->proxy.name || !*conn->proxy.name) {
      ...
      if(rc == CURLRESOLV_PENDING)
        *async = TRUE;
        ...
        failf(data, "Couldn't resolve host '%s'", conn->host.name);
        result =  CURLE_COULDNT_RESOLVE_HOST;
