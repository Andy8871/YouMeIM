...
static void commajoin(char **dst, const char *src)
{
  ...
  if (*dst)
  {
	  tmp = (char*)malloc(strlen(*dst) + strlen(src) + 2);
    if (!tmp)
      return;
...
static int get_DNS_NetworkParams(char **outptr)
{
  ...
  *outptr = NULL;
  ...
  newfi = (FIXED_INFO*)realloc(fi, size);
  if (!newfi)
    goto done;
...
static int get_DNS_AdaptersAddresses(char **outptr)
{
  ...
  *outptr = NULL;
  ...
  ipaa = (IP_ADAPTER_ADDRESSES*)malloc(Bufsz);
  if (!ipaa)
    return 0;
  ...
  while ((res == ERROR_BUFFER_OVERFLOW) && (--trying))
  {
    if (Bufsz < ReqBufsz)
    {
		newipaa = (IP_ADAPTER_ADDRESSES*)realloc(ipaa, ReqBufsz);
      if (!newipaa)
        goto done;
      Bufsz = ReqBufsz;
      ipaa = newipaa;
