                    ...
                    Curl_addrinfo **result)
{
  ...
  *result = NULL; /* assume failure */
  ...
  for(ai = aihead; ai != NULL; ai = ai->ai_next) {
    ...
    if((ca = (Curl_addrinfo*)malloc(sizeof(Curl_addrinfo))) == NULL) {
      error = EAI_MEMORY;
      break;
    }
    ...
    if((ca->ai_addr = (sockaddr*)malloc(ss_size)) == NULL) {
      error = EAI_MEMORY;
      free(ca);
      break;
    }
...
Curl_he2ai(const struct hostent *he, int port)
{
  ...
  for(i=0; (curr = he->h_addr_list[i]) != NULL; i++) {
    ...
    if((ai->ai_addr = (sockaddr*)calloc(1, ss_size)) == NULL) {
      result = CURLE_OUT_OF_MEMORY;
...
#pragma message disable PTRMISMATCH
  ...
  buf = (namebuff*)malloc(sizeof(struct namebuff));
  if(!buf)
    return NULL;
...
Curl_addrinfo *Curl_unix2addr(const char *path)
{
  ...
  if(!ai)
    return NULL;
  if((ai->ai_addr = (sockaddr*)calloc(1, sizeof(struct sockaddr_un))) == NULL) {
    free(ai);
    return NULL;
  }
