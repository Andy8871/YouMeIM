...
{
  struct SessionHandle *data = conn->data;
  struct FTP *ftp = (FTP *)data->req.protop;
...
{
  CURLcode result;
  struct FTP *ftp = (FTP *)conn->data->req.protop;
  /* send USER */
                            ...
                            bool *magicdone)
{
  ...
  *magicdone = FALSE;
  ...
  if(conn->bits.tunnel_proxy && conn->bits.httpproxy) {
	...
	struct FTP *ftp_save = (FTP*)data->req.protop;
	...
	if (conn->tunnel_state[SECONDARYSOCKET] != connectdata::TUNNEL_COMPLETE) {
...
static CURLcode ftp_do_more(struct connectdata *conn, int *completep)
{
  ...
  if(!conn->bits.tcpconnect[SECONDARYSOCKET]) {
	  if (conn->tunnel_state[SECONDARYSOCKET] == connectdata::TUNNEL_CONNECT) {
...
static void wc_data_dtor(void *ptr)
{
	struct ftp_wc_tmpdata *tmp = (ftp_wc_tmpdata*)ptr;
  if(tmp)
...
static CURLcode init_wc_data(struct connectdata *conn)
{
  ...
  ftp_tmp = (ftp_wc_tmpdata*)calloc(1, sizeof(struct ftp_wc_tmpdata));
  if(!ftp_tmp) {
  ...
  ftp_tmp->backup.file_descriptor =(FILE*) conn->data->set.out;
...
static CURLcode wc_statemach(struct connectdata *conn)
{
  ...
  case CURLWC_MATCHING: {
    /* In this state is LIST response successfully parsed, so lets restore
	  ...
	  struct ftp_wc_tmpdata *ftp_tmp = (ftp_wc_tmpdata*)wildcard->tmp;
  ...
  case CURLWC_DOWNLOADING: {
    ...
    struct ftp_conn *ftpc = &conn->proto.ftpc;
	struct curl_fileinfo *finfo = (curl_fileinfo *)wildcard->filelist->head->ptr;
...
static CURLcode ftp_setup_connection(struct connectdata *conn)
{
  ...
  conn->data->req.protop = ftp = (FTP*)malloc(sizeof(struct FTP));
  if(NULL == ftp)
    return CURLE_OUT_OF_MEMORY;
