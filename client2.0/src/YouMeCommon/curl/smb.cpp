...
{
	struct smb_request *req = (smb_request *)conn->data->req.protop;
#if defined(DEBUGBUILD) && !defined(CURL_DISABLE_VERBOSE_STRINGS)
  /* For debug purposes */
  static const char * const names[] = {
    "SMB_REQUESTING",
...
static CURLcode smb_setup_connection(struct connectdata *conn)
{
  ...
  conn->data->req.protop = req = (smb_request*)calloc(1, sizeof(struct smb_request));
  if(!req)
...
static CURLcode smb_connection_state(struct connectdata *conn, bool *done)
{
  ...
  case SMB_NEGOTIATE:
    ...
    nrsp =(smb_negotiate_response*) msg;
...
static CURLcode smb_parse_url_path(struct connectdata *conn)
{
  ...
  struct smb_request *req = (smb_request *)data->req.protop;
  char *path;
  char *slash;
