...
public:
	...
	XString NameXString() const;
	const char*		Value() const		{ return value.c_str (); }		///< Return the value of this attribute.
	XString ValueXString() const;
...
{
public:
	/// Construct an element.最好不要用这个，这个函数不支持跨平台
	...
	*/
	XString Attribute( const char* name ) const;
	...
	/** Given an attribute name, Attribute() returns the value
		...
		is non-null.
	*/
	XString Attribute( const char* name, int* i ) const;
...
#ifdef WIN32
	TiXmlText (const wchar_t * initValue );
#endif
	virtual ~TiXmlText() {}
