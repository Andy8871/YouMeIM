...
{
	struct site_blacklist_entry *entry = (site_blacklist_entry*) element;
  (void)user;
                             ...
                             struct connectdata *conn)
{
  if(data) {
    ...
    if(conn->recv_pipe && conn->recv_pipe->head) {
		struct SessionHandle *recv_handle = (SessionHandle *)conn->recv_pipe->head->ptr;
  ...
  if(handle->multi) {
    ...
    if(blacklist) {
      ...
      while(curr) {
        ...
        site = (site_blacklist_entry*)curr->ptr;
                                           ...
                                           struct curl_llist **list_ptr)
{
  ...
  if(sites) {
    ...
    while(*sites) {
	  ...
	  entry = (site_blacklist_entry*)malloc(sizeof(struct site_blacklist_entry));
      if(!entry) {
        free(hostname);
        Curl_llist_destroy(new_list, NULL);
        return CURLM_OUT_OF_MEMORY;
