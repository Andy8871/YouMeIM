...
CURLcode init_telnet(struct connectdata *conn)
{
  ...
  tn =(TELNET*) calloc(1, sizeof(struct TELNET));
  if(!tn)
  ...
  */
  ...
  while(keepon) {
    ...
    case WAIT_TIMEOUT:
    {
      for(;;) {
        if(data->set.is_fread_set) {
          ...
          result = (CURLcode) conn->fread_func(buf, 1, BUFSIZE - 1, conn->fread_in);
    ...
    case WAIT_OBJECT_0:
	  ...
	  if (SOCKET_ERROR == WSAEnumNetworkEvents(sockfd, event_handle, &events)) {
