                                         ...
                                         struct SessionHandle *data)
{
  struct Curl_sh_entry *there =
	  (Curl_sh_entry*) Curl_hash_pick(sh, (char *)&s, sizeof(curl_socket_t));
                                     ...
                                     int chashsize) /* connection hash */
{
  ...
  multi->closure_handle = (SessionHandle*)curl_easy_init();
                                ...
                                CURL *easy_handle)
{
  ...
  if((data->set.global_dns_cache) &&
	  (data->dns.hostcachetype != Names::HCACHE_GLOBAL)) {
  ...
  /* for multi interface connections, we share DNS cache automatically if the
	...
	data->dns.hostcachetype = Names::HCACHE_MULTI;
  }
  ...
  data->multi = (Curl_multi *)multi_handle;
...
{
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
  struct SessionHandle *easy = (SessionHandle*)curl_handle;
  ...
  if (data->dns.hostcachetype == Names::HCACHE_MULTI) {
  ...
  for(e = multi->msglist->head; e; e = e->next) {
	  struct Curl_message *msg = (Curl_message*) e->ptr;
                                    ...
                                    int numsocks)
{
  ...
  if (conn->tunnel_state[FIRSTSOCKET] == connectdata::TUNNEL_CONNECT)
          ...
          if(async)
            ...
            if(protocol_connect)
              multistate(data, multi->pipelining_enabled?
				...
				if (data->easy_conn->tunnel_state[FIRSTSOCKET] == connectdata::TUNNEL_CONNECT)
    ...
    case CURLM_STATE_WAITPROXYCONNECT:
      ...
      if(data->easy_conn->bits.proxy_connect_closed) {
        ...
        multistate(data, CURLM_STATE_CONNECT);
		  ...
		  if (data->easy_conn->tunnel_state[FIRSTSOCKET] == connectdata::TUNNEL_COMPLETE)
    ...
    case CURLM_STATE_PERFORM:
    {
      ...
      if(result) {
        ...
        if(data->easy_conn->recv_pipe->head)
			Curl_expire_latest((SessionHandle*)data->easy_conn->recv_pipe->head->ptr, 1);
...
CURLMcode curl_multi_perform(CURLM *multi_handle, int *running_handles)
{
  ...
  do {
    ...
    if(t)
      ...
      (void)add_next_timeout(now, multi, (SessionHandle*)t->payload);
                         ...
                         struct SessionHandle *data)
{
  ...
  for(i=0; (i< MAX_SOCKSPEREASYHANDLE) &&
	...
	entry = (Curl_sh_entry*)Curl_hash_pick(multi->sockhash, (char *)&s, sizeof(s));
          ...
          if(easy_conn->recv_pipe && easy_conn->recv_pipe->size > 1) {
            ...
            if(entry->easy == data) {
              if(isHandleAtHead(data, easy_conn->recv_pipe))
                entry->easy =(SessionHandle*) easy_conn->recv_pipe->head->next->ptr;
              else
				  entry->easy = (SessionHandle*)easy_conn->recv_pipe->head->ptr;
            }
          }
          if(easy_conn->send_pipe  && easy_conn->send_pipe->size > 1) {
            ...
            if(entry->easy == data) {
              if(isHandleAtHead(data, easy_conn->send_pipe))
				  entry->easy = (SessionHandle*)easy_conn->send_pipe->head->next->ptr;
              else
				  entry->easy = (SessionHandle*)easy_conn->send_pipe->head->ptr;
            }
          }
...
void Curl_multi_closed(struct connectdata *conn, curl_socket_t s)
{
  ...
  if(multi) {
    ...
    struct Curl_sh_entry *entry =
		(Curl_sh_entry*)Curl_hash_pick(multi->sockhash, (char *)&s, sizeof(s));
                              ...
                              int *running_handles)
{
    ...
    if(!entry)
      ...
      if(data->easy_conn) {
        if((ev_bitmask & CURL_POLL_OUT) &&
		   ...
		   data = (SessionHandle*)data->easy_conn->send_pipe->head->ptr;
        else if((ev_bitmask & CURL_POLL_IN) &&
                data->easy_conn->recv_pipe &&
                data->easy_conn->recv_pipe->head)
				data = (SessionHandle*)data->easy_conn->recv_pipe->head->ptr;
      }
  ...
  do {
    ...
    multi->timetree = Curl_splaygetbest(now, multi->timetree, &t);
    if(t) {
		data = (SessionHandle*)t->payload; /* assign this for next loop */
		(void)add_next_timeout(now, multi, (SessionHandle*)t->payload);
    }
                 ...
                 struct timeval *stamp)
{
  ...
  timedup =(timeval*) malloc(sizeof(*timedup));
  if(!timedup)
    return CURLM_OUT_OF_MEMORY;
  ...
  if(Curl_llist_count(timeoutlist)) {
    ...
    for(e = timeoutlist->head; e; e = e->next) {
		struct timeval *checktime = (timeval*) e->ptr;
                            ...
                            curl_socket_t s, void *hashp)
{
  ...
  if(s != CURL_SOCKET_BAD)
	  there = (Curl_sh_entry*)Curl_hash_pick(multi->sockhash, (char *)&s, sizeof(curl_socket_t));
...
void Curl_multi_process_pending_handles(struct Curl_multi *multi)
{
  ...
  while(e) {
	  struct SessionHandle *data = (SessionHandle *)e->ptr;
