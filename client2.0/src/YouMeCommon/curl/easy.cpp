...
{
    va_list arg;
	struct SessionHandle *data = (SessionHandle*)curl;
    CURLcode result;
...
static CURLcode easy_perform (struct SessionHandle *data, bool events)
{
    ...
    else
    {
		...
		data->multi_easy = (Curl_multi *)multi;
    }
...
{
    return easy_perform ((SessionHandle*)easy, FALSE);
}
...
CURL *curl_easy_duphandle (CURL *incurl)
{
	...
	struct SessionHandle *outcurl = (SessionHandle *)calloc(1, sizeof(struct SessionHandle));
    if (NULL == outcurl)
        goto fail;
...
static CURLcode easy_connection (struct SessionHandle *data, curl_socket_t *sfd, struct connectdata **connp)
{
    if (data == NULL)
        return CURLE_BAD_FUNCTION_ARGUMENT;
