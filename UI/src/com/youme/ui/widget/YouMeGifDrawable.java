...
import com.youme.ui.utils.YouMeGifDecoder;
...
public class YouMeGifDrawable extends AnimationDrawable {

    private int mCurrentIndex = 0;
    private UpdateListener mListener;

    public YouMeGifDrawable(){
    }
    
    public YouMeGifDrawable(UpdateListener listener){
    	mListener = listener;
    }
    public YouMeGifDrawable(InputStream source, UpdateListener listener) {
        mListener = listener;
        YouMeGifDecoder decoder = new YouMeGifDecoder();
        decoder.read(source);
    ...
    public void setFrames(List<Bitmap> frames, int framesDuration){
    	for (int i = 0; i < frames.size(); ++i){
    		Bitmap bitmap = frames.get(i);
    		BitmapDrawable drawable = new BitmapDrawable(bitmap);
    		drawable.setBounds(0, 0, bitmap.getWidth(), bitmap.getHeight());
    		addFrame(drawable, framesDuration);
    		if (0 == i){
    			setBounds(0, 0, bitmap.getWidth(), bitmap.getHeight());
    		}
    	}
